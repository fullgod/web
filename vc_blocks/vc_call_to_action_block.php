...
function ebor_call_to_action_shortcode( $atts ) {
	...
	$output = '<div class="row clearfix">
		<div class="col-sm-6 col-xs-12 pull-left">
			<h3 class="text-white">'.  htmlspecialchars_decode($lead) .'</h3>
		</div>
		
		<div class="col-sm-4 col-xs-12 pull-right text-right">
			<a href="'. esc_url($text) .'" class="btn btn-primary btn-white">'.  htmlspecialchars_decode($title) .'</a>
		</div>
	</div>';
