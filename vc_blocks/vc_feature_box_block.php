...
function ebor_feature_box_shortcode( $atts, $content = null ) {
	...
	if( is_array($slides) ){
		...
		if( $sub )
			$output .= '<span class="alt-font text-white">'. htmlspecialchars_decode($sub) .'</span>';
			
		if( $title )
			$output .= '<h1 class="text-white">'. htmlspecialchars_decode($title) .'</h1>';
	
		if( $content )
			$output .= '<div class="text-white">'. wpautop(do_shortcode(htmlspecialchars_decode($content))) .'</div>';
		
		if( $link || $button_text )
			$output .= '<a href="'. esc_url($link) .'" class="btn btn-primary btn-white" target="'. esc_attr($target) .'">'. htmlspecialchars_decode($button_text) .'</a>';
